...
Route::get('/', function () {
    ...
    $logo_setting = Setting::find(23);
    return view('page.login', ['contents' => json_decode(FrontEnd::find(1)->content), 'logo_setting' => $logo_setting]);
});
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::post('/patient/{patient}/discharge', [PatientDetailController::class, 'discharge'])->name('patient-details.discharge');
    Route::get('/patients/discharged', [PatientDetailController::class, 'dischargedPatients'])->name('patients.discharged');
    Route::post('/patients/{patient}/mark-old', [PatientDetailController::class, 'markAsOld'])->name('patients.mark-old');
    Route::get('/old-patients', [PatientDetailController::class, 'oldPatients'])->name('patients.old');
    Route::post('/patient/{id}/update-status', [PatientDetailController::class, 'updateStatus'])->name('patient.update.status');
    Route::post('/patient/{id}/update-discharge-status', [PatientDetailController::class, 'changeStatus'])->name('patient.update-discharge.status');
    Route::post('/patients/update-payment', [PatientDetailController::class, 'updatePayment'])->name('patients.update-payment');
    Route::get('/patient/{patient}/attachments', [PatientDetailController::class, 'showAttachments'])->name('patient-details.attachments');
    Route::get('/patient-details/{id}/attachments', [PatientDetailController::class, 'showAttachments'])->name('patient-details.attachments');
    Route::get('/patient/{id}/upload-attachments', [PatientDetailController::class, 'uploadAttachments'])->name('patient-details.upload-attachments');
    Route::post('/patient/{id}/attachments/store', [PatientDetailController::class, 'storeAttachments'])->name('patient-details.store.attachments');
    Route::resources([
